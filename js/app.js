(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".video-swiper__slider")) new core(".video-swiper__slider", {
            ...
            if (document.querySelector(".popular__slider")) new core(".popular__slider", {
                modules: [ Lazy, Navigation, Scrollbar ],
                observer: true,
                observeParents: true,
                spaceBetween: 20,
                autoHeight: false,
            ...
            if (document.querySelector(".route-description__slider")) new core(".route-description__slider", {
                modules: [ Lazy, Navigation, Scrollbar ],
                observer: true,
                observeParents: true,
                slidesPerView: 1.5,
...
***************************************************************************** */
            ...
            galleries.forEach((gallery => {
                galleyItems.push({
                    ...
                    galleryClass: lightgallery_es5(gallery, {
                        selector: ".row-gallery__image-ibg",
                        licenseKey: "7EC452A9-0CFD441C-BD984C7C-17C8456E",
                        speed: 500,
                        controls: true
                    })
                });
                console.log(galleyItems[0]);
            }));
        ...
        const videoItems = document.querySelectorAll(".slide-swiper-video__item");
        const videoControlButton = document.querySelectorAll(".slide-swiper-video__control");
        const likeButtons = document.querySelectorAll(".like-button");
        let itemsTravelBody = document.querySelectorAll(".body-tabs-excursions__items");
        let travelCards = document.querySelectorAll(".item-tabs-excursions");
        let buttonFilter = document.querySelectorAll("[data-change-button]");
        let resetButton = document.querySelector(".filters-excursions__reset");
        let durationList = document.querySelectorAll('[name="time"]');
        let minPrice = document.querySelector("#price-start");
        let maxPrice = document.querySelector("#price-end");
        let checkedDuration;
        document.addEventListener("click", (e => {
            for (let i = 0; i < videoItems.length; i++) if (e.target == videoItems[i] || e.target == videoControlButton[i]) if (videoItems[i].paused) {
                videoItems[i].play();
                videoControlButton[i].setAttribute("hidden", "hidden");
            } else {
                videoItems[i].pause();
                videoControlButton[i].removeAttribute("hidden");
            }
            for (let button of likeButtons) if (e.target == button) {
                if (e.target.closest("[data-delete-item]") && e.target.classList.contains("_liked")) e.target.closest("[data-delete-item]").remove();
                button.classList.toggle("_liked");
            }
            for (let button of buttonFilter) {
                if (e.target == button) for (let item of durationList) if (item.checked) {
                    checkedDuration = item.value;
                    console.log(checkedDuration);
                }
                for (let card of travelCards) if (e.target == button) if (checkedDuration == card.dataset.duration && card.dataset.location == originalSelectLocation.value && card.dataset.persons == selectOriginalCountOfPeople.value && +card.dataset.price >= minPrice.value && +card.dataset.price <= maxPrice.value) card.style.display = "flex"; else card.style.display = "none"; else if (e.target == resetButton) card.style.display = "flex";
                for (let list of itemsTravelBody) {
                    console.log(list);
                }
            }
        }));
        let savedCards = document.querySelector(".content-information-block__items");
        if (savedCards) {
            const callback = (mutationsList, observer) => {
                if (!savedCards.children.length) savedCards.innerHTML = `<p class="special-text">Нет сохраненных путешествий</p>`;
            };
            const observer = new MutationObserver(callback);
            observer.observe(savedCards, {
                childList: true
            });
        }
        let numberOfAdults = document.querySelector("#adults");
        let numberOfChildren = document.querySelector("#children");
        let getValue = someInput => someInput.value;
        if (numberOfAdults) {
            numberOfAdults.addEventListener("blur", (() => {
                let number;
                number = getValue(numberOfAdults);
                1 == number ? numberOfAdults.value = `${number} взрослый` : "" == number ? numberOfAdults.value = "" : numberOfAdults.value = `${number} взрослых`;
            }));
            numberOfAdults.addEventListener("focus", (() => {
                numberOfAdults.value = "";
            }));
        }
        if (numberOfChildren) {
            numberOfChildren.addEventListener("blur", (() => {
                let number;
                number = getValue(numberOfChildren);
                1 == number ? numberOfChildren.value = `${number} ребенок` : "" == number ? numberOfChildren.value = "" : numberOfChildren.value = `${number} детей`;
            }));
            numberOfChildren.addEventListener("focus", (() => {
                numberOfChildren.value = "";
            }));
        }
        let locationsSelect = document.querySelectorAll(".select__option.location-option");
        let locationsRadio = document.querySelectorAll('input[name="location"]');
        let originalSelectLocation = document.querySelector("#location");
        for (const i in locationsRadio) if (locationsRadio.hasOwnProperty(i)) locationsRadio[i].addEventListener("change", (function() {
            let checkedLocation = document.querySelector(".select__content.location-option");
            let checkedOption = document.querySelector("button[hidden].select__option.location-option");
            changeCheckedElement(checkedLocation, checkedOption, locationsSelect[i], originalSelectLocation);
        }));
        for (let locationSel of locationsSelect) locationSel.addEventListener("click", (e => {
            for (let radioButton of locationsRadio) if (e.target.dataset.value == radioButton.value) radioButton.checked = true;
        }));
        let countOfPeopleSelect = document.querySelectorAll(".select__option.people-option");
        let countOfPeopleRadio = document.querySelectorAll('input[name="people"]');
        let selectOriginalCountOfPeople = document.querySelector("#countOfPeople");
        for (const i in countOfPeopleRadio) if (countOfPeopleRadio.hasOwnProperty(i)) countOfPeopleRadio[i].addEventListener("change", (function() {
            let checkedCount = document.querySelector(".select__content.people-option");
            let checkedOption = document.querySelector("button[hidden].select__option.people-option");
            changeCheckedElement(checkedCount, checkedOption, countOfPeopleSelect[i], selectOriginalCountOfPeople);
        }));
        for (let persons of countOfPeopleSelect) persons.addEventListener("click", (e => {
            for (let radioButton of countOfPeopleRadio) if (e.target.dataset.value == radioButton.value) radioButton.checked = true;
        }));
        function changeCheckedElement(checkedTitle, checkedOption, selectItem, orginalSelect) {
            checkedOption.removeAttribute("hidden");
            checkedTitle.innerHTML = selectItem.dataset.value;
            selectItem.setAttribute("hidden", "hidden");
            orginalSelect.value = selectItem.dataset.value;
        }
        ...
        if (document.querySelector("#datepicker")) {
            const inputPicker = datepicker_min("#datepicker", {
                customDays: [ "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" ],
                customMonths: [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ],
                overlayButton: "Apply",
                overlayPlaceholder: "Year (4 digits)",
                ...
                formatter: (input, date, instance) => {
                    const value = date.toLocaleDateString();
                    dayValye = input.value = value;
                },
                onSelect: function(input, instance, date) {
                    for (let inpDatepicker of document.querySelectorAll("input[data-same-date]")) inpDatepicker.value = dayValye;
                }
            });
            flsModules.datepicker = inputPicker;
        }
        if (document.querySelector("#datepickerTwo")) {
            const inputPicker = datepicker_min("#datepickerTwo", {
